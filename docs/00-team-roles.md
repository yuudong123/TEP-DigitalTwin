...
| B | Infrastructure / Operations | 08 Docker 개발환경 유지·보완, 17 운전상태·열화 변화 모니터링 설계 | 09 수동 배포 검증, 10 Jenkins 실운영 설정, 20 모델 적용, 21·16 일부 통합 |
...
### B — 운영 기반 및 변화 모니터링 설계

- Docker Compose는 구성 완료 상태이며, API·inference·monitor 실행 모듈이
  구현되면 전체 기동을 다시 검증한다.
- 17번에서는 변화 event schema를 정하고, 운영 Drift 데이터가 확보되기 전까지
  재학습 trigger 연계는 보류한다.
...
- 18·19번은 운영 Drift 데이터·label과 B의 변화 event, A의 inference 결과 schema가
  확정된 뒤 범위를 다시 정해 구현한다.
...
11 Kafka 재생 ──→ 12 Inference ──→ 13 FastAPI ──→ 14 Web / 15 Unity
                     │
                     └──────────→ 17 변화 Monitor ──→ 18·19 운영 Drift 데이터 확보 후 재검토 ──→ 20 적용
...
- B: Compose 환경변수, 로그·배포 규칙, 변화 event와 모델 적용 규칙
